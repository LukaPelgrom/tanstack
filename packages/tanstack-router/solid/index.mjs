...
function createSolidMutableStore(initialValue) {
  const [signal, setSignal] = createSignal(initialValue, { ownedWrite: true });
...
var getStoreFactory = (options) => {
  var _a;
  if ((_a = isServer) != null ? _a : options.isServer) {
    return {
...
var NativeSolidRouter = class extends RouterCore {
  constructor(options) {
...
var createRoute = ((options) => new BaseRoute(options));
var createRootRoute = ((options) => new BaseRootRoute(options));
...
var getRouteApi = ((id) => new BaseRouteApi({ id }));
var RouteApi = BaseRouteApi;
    ...
    dispose = render(
      () => createComponent2(RouterContextProvider, {
        ...
        children: () => createComponent2(ErrorBoundary, {
          ...
          children: createComponent2(SafeRouteView, {})
        })
      }),
      page
    );
...
function setupBackHandler(router, _getFrame, _guard) {
  ...
  if (isAndroid) {
    ...
    (_a = Application.android) == null ? void 0 : _a.on("activityBackPressed", handler);
    cleanups.push(() => {
      var _a2;
      (_a2 = Application.android) == null ? void 0 : _a2.off("activityBackPressed", handler);
    });
  }
  return () => {
...
function NativeScriptRouterProvider(props) {
  ...
  const frameEl = document2.createElement("frame");
  frameRef = frameEl;
...
// src/Link.tsx
import { createMemo as createMemo2, createRenderEffect, onCleanup } from "solid-js";
...
function Link(props) {
  ...
  const contentView = createElement("contentview");
